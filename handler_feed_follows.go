...
func (apiConfig *apiConfig) handlerCreateFeedFollows(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	feed, err := apiConfig.DB.CreateFeedFollow(r.Context(), database.CreateFeedParams{
		ID:        uuid.New(),
	...
	respondeWithJson(w, 201, databaseFeedToFeed(feed))
}
...
func (apiConfig *apiConfig) handlerGetFeeds(w http.ResponseWriter, r *http.Request) {
	...
	respondeWithJson(w, 201, databaseFeedsToFeeds(feeds))
}
